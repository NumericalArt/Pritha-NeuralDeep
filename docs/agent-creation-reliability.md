...
Для `researchSelectionVersion: 2` хост выдаёт отдельные
...
Повторная реальная проверка Wikipedia выявила потерю содержания при отборе:
параметры `srsearch` и `srlimit` присутствовали на прочитанной странице, но
короткие имена отделялись от описаний и выпадали из построчного excerpt.
После возврата строк в порядок страницы первые восемь passages занимала
навигация. Модель правильно отметила недостаток сведений; автоматическая
коррекция получила те же источники и не могла восполнить пропущенные данные.

Новые jobs закрепляют selection v3. Хост ранжирует непрерывные абзацы вместе
с описаниями параметров, исключает блоки навигации до разрезания длинного текста
и покрывает разные слова запроса до повторяющихся вариантов одного параметра.
Лимиты остаются прежними: максимум восемь passages и 2400 bytes цитат на
страницу. Цитаты не переписываются, их hash и принадлежность источнику
проверяются. Пустая по содержанию страница расходует одну из двух прежних
попыток получения источника; модель до получения пригодных сведений не вызывается.

Если ответ явно указывает unknown/incompatible и обязательные темы не закрыты,
selection v3 возвращает `creation_research_evidence_incomplete` без платной
коррекции того же пакета. История ответа и страницы сохраняются, ложная
публикация research не происходит. Ошибки формата по-прежнему допускают одну
ограниченную коррекцию. Старые selection v1/v2 не мигрируют и воспроизводят
исходный набор цитат. Проверки включают реальные сохранённые публичные страницы,
навигацию, многострочные параметры, Unicode, чужие IDs и остановку gateway.

В следующем запросе той же реальной проверки буферизованный Responses stream
закончился без terminal event. Чтение тела завершилось, локальный deadline не
сработал (`timedOut: false`), а итоговый usage отсутствовал. Причина потери
события у провайдера или посредника не установлена. Прежний normalizer выбрасывал
обычный Error, поэтому оператор видел `network_error` вместо нарушения протокола.
Теперь обе ветви normalizer используют `neuraldeep_stream_truncated` и
`neuraldeep_stream_malformed`; автоматический повтор запрещён, неизвестный
расход не заменяется нулём. Сообщение об ошибке JSON не включает фрагмент
сырого ответа. Регрессионные проверки подтверждают один upstream request,
неизвестный usage, отсутствие ложного timeout и сохранение закрытия соединения.

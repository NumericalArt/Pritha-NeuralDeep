...
import { closeSync, cpSync, existsSync, lstatSync, mkdirSync, openSync, readFileSync, readSync, readdirSync, realpathSync, writeFileSync } from "node:fs";
...
const finderHeader = Buffer.from([0, 0, 0, 1, 66, 117, 100, 49]);
function finderMetadata(file, stat) {
  if (path.basename(file) !== ".DS_Store" || !stat.isFile() || stat.size < finderHeader.length) return false;
  const fd = openSync(file, "r"), header = Buffer.alloc(finderHeader.length);
  try { return readSync(fd, header, 0, header.length, 0) === header.length && header.equals(finderHeader); }
...
// Legacy digests cover Finder metadata too. Version 2 excludes only regular
// folder-view metadata; every executable, chunk, manifest and identity remains covered.
export function buildTreeDigest(directory, { excludeFinderMetadata = false } = {}) {
  ...
  function visit(relative) {
    for (const name of readdirSync(path.join(root, relative)).sort()) {
      ...
      if (excludeFinderMetadata && finderMetadata(file, stat)) continue;
...
export function prepareRollbackArtifact(source, destination, required = ND_STORAGE_COMPATIBILITY) {
  ...
  cpSync(source, build, { recursive: true, filter: file => path.relative(source, file).split(path.sep)[0] !== "cache"
    && !finderMetadata(file, lstatSync(file)) });
  const receipt = { schema: "pritha-rollback-artifact-v1", identity,
    digest: { version: 2, ...buildTreeDigest(build, { excludeFinderMetadata: true }) } };
...
export function verifyRollbackArtifact(directory, required = ND_STORAGE_COMPATIBILITY) {
  ...
  if (receipt.digest?.version !== undefined && receipt.digest.version !== 2) throw new Error("Unsupported rollback digest version");
  const identity = readBuildIdentity(build), digest = buildTreeDigest(build, { excludeFinderMetadata: receipt.digest?.version === 2 });

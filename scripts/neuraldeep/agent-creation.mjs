...
export function creationPrompt(job) {
  ...
  if(job.preparationPolicyVersion===2 && phase==='research')return [
    ...
    `If a particular research section needs detail, use the bounded reader: node ${quote(path.join(job.executionCodeRoot || '.', 'scripts/creation-context-reader.mjs'))} --packet ${job.contextPacket?.hash} --artifact <research|patterns> --hash <contentHash from the packet> --cursor <evidenceRef.cursor, 0 or nextCursor>. Advisory memory matches retain an exact section reference; read only relevant sections before relying on them. Each page includes hashes and an explicit continuation marker. Do not reread the same page.`,

import { VOICE_SCHEMA, NeuralDeepVoiceJournal } from "./voice-journal.mjs";
...
export class NeuralDeepCoordinationStore {
  constructor({ databasePath = ":memory:", legacyPath = null, workerId = `worker_${randomUUID()}` } = {}) {
    ...
    this.voiceJournal = new NeuralDeepVoiceJournal(this);
    try {
    if (databasePath !== ":memory:") chmodSync(databasePath, 0o600);
    this.db.exec("PRAGMA busy_timeout=5000; PRAGMA journal_mode=WAL; PRAGMA synchronous=FULL;");
    const version = this.db.prepare("PRAGMA user_version").get().user_version;
    if (![0, 1, 2, 3, 4].includes(version)) {throw new Error("admission_schema_unsupported");}
    this.transaction(() => {
  ...
  enqueue(input) {
    ...
    if (input.surface === "voice_dialogue" && (resources.length || input.sessionKeyHash || input.resumePausedKey || input.predecessorPriority)) throw new Error("admission_dialogue_scope_invalid");
  ...
  _bindSession(id, token, sessionKeyHash) {
      ...
      if (current.surface === "voice_dialogue") throw new Error("admission_dialogue_scope_invalid");
  ...
  reconcileWorkload(workload, outcome = "failed") {
    ...
    return this.transaction(() => this._reconcileWorkload(workload,outcome));
  }

  _reconcileWorkload(workload, outcome) {
      ...
      for (const row of rows) {
        if (["active","queued","resume_confirmation_required","waiting_for_provider","waiting_for_operator"].includes(row.status) || (row.status === "failed" && outcome === "cancelled")) {
  ...
  reconcileDeadWorkers() {
    return this.transaction(() => {
      ...
      for (const row of rows) {
        ...
        if (!absent) continue;
        if (row.surface === "voice_dialogue") {
          const finishedAt = new Date().toISOString();
          this.db.prepare("UPDATE attempts SET status='cancelled',finished_at=? WHERE id=?").run(finishedAt,row.id);
          this.db.prepare("UPDATE voice_http_requests SET status='interrupted',finished_at=?,accounting_pending=1 WHERE id=? AND status='dispatching'").run(finishedAt,row.id);
          count += 1;
          continue;
        }
      ...
      const dead = pid => {if(!Number.isSafeInteger(pid)||pid<1)return false;try{process.kill(pid,0);return false;}catch(error){return error.code === 'ESRCH';}};
      for (const row of this.db.prepare("SELECT id,metadata FROM voice_http_requests WHERE status='dispatching'").all()) {
        if (dead(JSON.parse(row.metadata).workerPid)) this.db.prepare("UPDATE voice_http_requests SET status='interrupted',finished_at=?,accounting_pending=1 WHERE id=?").run(new Date().toISOString(),row.id);
      }
      for (const row of this.db.prepare("SELECT id,worker_pid FROM voice_operations WHERE status='accepted'").all()) {
        if (dead(row.worker_pid)) this.db.prepare("UPDATE voice_operations SET status='recovery_required' WHERE id=?").run(row.id);
      }
      for (const row of this.db.prepare("SELECT session_id,id,worker_pid FROM voice_turns WHERE status='accepted'").all()) {
        if (dead(row.worker_pid)) this.db.prepare("UPDATE voice_turns SET status='interrupted' WHERE session_id=? AND id=?").run(row.session_id,row.id);
      }
      return count;
    });
  }

  // Release abandoned direct-chat launches, never replay them. The history store
  // retains the interrupted turn and its explicit Resume/Retry controls. A PID
  // alone is insufficient: every dispatched attempt must have process-tree proof.
  reconcileStoppedTaskChats() {
    return this.transaction(() => {
      const candidates = this.db.prepare(`SELECT DISTINCT workload FROM attempts
        WHERE surface='task_chat' AND status IN ('failed','resume_confirmation_required')`).all();
      let released = 0;
      for (const { workload } of candidates) {
        const rows = this.db.prepare('SELECT * FROM attempts WHERE workload=?').all(workload);
        if (rows.some(row => row.surface !== 'task_chat' || ['active','queued','waiting_for_operator','waiting_for_provider'].includes(row.status)
          || JSON.parse(row.payload || '{}').execution?.voiceHandoff)) continue;
        const scopes = [...new Set(rows.flatMap(row => [row.scope,row.session_scope]).filter(Boolean))];
        if (scopes.some(scope => this.db.prepare('SELECT 1 FROM logical_owners WHERE scope=? AND held=1').get(scope)
          || this.db.prepare('SELECT 1 FROM handoff_scopes WHERE scope=?').get(scope)
          || this.db.prepare("SELECT 1 FROM paused_scopes WHERE scope=? AND reason NOT IN ('failed','runtime_exit_unconfirmed','worker_exit_requires_reconciliation','resume_confirmation_required')").get(scope))) continue;
        const unfinished = rows.filter(row => ['failed','resume_confirmation_required'].includes(row.status));
        if (!unfinished.length || unfinished.some(row => {
          const runtime = this.db.prepare('SELECT run_id FROM runtime_owners WHERE admission_id=?').get(row.id);
          return !runtime || this.runtimeRun(runtime.run_id)?.process_protocol !== 1 || !this._reconcileRuntimeRunExit(runtime.run_id);
        })) continue;
        // reconcileWorkload rechecks all earlier attempts and releases only this
        // workload's claims. Its immutable receipts and usage remain available.
        try { this._reconcileWorkload(workload,'cancelled'); }
        catch (error) { if (error.message === 'admission_runtime_exit_unconfirmed') continue; throw error; }
        this.db.prepare('INSERT OR REPLACE INTO coordination_meta(key,value) VALUES(?,?)')
          .run(`stopped-chat-cleanup:${workload}`, JSON.stringify({at:new Date().toISOString(),reason:'process_tree_exit_verified'}));
        released++;
      }
      return released;
    });
  ...
  acquireSessionControl(scope, owner, resources=[]) {
    if (!SCOPE.test(scope) || !ID.test(owner)) throw new Error("admission_owner_invalid");
    ...
    return this.transaction(() => {
      ...
      if(claims.length && this.db.prepare("SELECT 1 FROM session_controls WHERE owner=?").get(owner))return false;
      ...
      if (prior) {
        ...
        // A dead server is not proof that a host tool it started also exited.
        // Preserve its claims until the exact action has been reconciled.
        if(this.db.prepare("SELECT 1 FROM resource_claims WHERE surface='host' AND workload=? AND held=1").get(prior.owner))return false;
      ...
      if(this._claimsConflict(claims,"host",owner))return false;
      this.db.prepare("INSERT INTO session_controls(scope,owner,worker_pid) VALUES(?,?,?)").run(scope, owner, process.pid);
      for(const claim of claims)this.db.prepare("INSERT INTO resource_claims VALUES(?,'host',?,?,?,?,1)").run(`host_${owner}`,owner,claim.kind,claim.key,claim.mode);
      return true;
    });
  }
  ...
  releaseSessionControl(scope, owner) {
    return this.transaction(()=>{
      const released=this.db.prepare("DELETE FROM session_controls WHERE scope=? AND owner=?").run(scope, owner).changes > 0;
      if(released)this.db.prepare("UPDATE resource_claims SET held=0 WHERE surface='host' AND workload=?").run(owner);
      return released;
    });
  }

  heldHostControls() {
    return this.db.prepare("SELECT scope,owner FROM session_controls WHERE EXISTS (SELECT 1 FROM resource_claims WHERE surface='host' AND workload=session_controls.owner AND held=1)").all();
  }

  _resourceConflict(row) {
    const resources=this.db.prepare("SELECT kind,resource AS key,mode FROM attempt_resources WHERE attempt_id=?").all(row.id);
    return this._claimsConflict(resources,row.surface,row.workload);
  }

  _claimsConflict(resources,surface,workload) {

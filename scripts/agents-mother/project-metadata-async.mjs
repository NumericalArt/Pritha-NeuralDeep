...
export function unavailableProjectMetadata(reason = "project-metadata-unavailable") {
  ...
  return { schema: "pritha-project-metadata-v1", manifest: { manifest: null, present: null, issue: reason }, envExample: file(), envLocal: file() };
}

export function projectMetadataIssueMessage(issue) {
  if (!issue) return null;
  if (issue === "project-metadata-timeout" || issue === "project-metadata-unavailable") {
    return "Agent status is temporarily unavailable. Reload to retry.";
  }
  return "operations/manifest.json is invalid or unsafe to read.";
}
...
export async function readProjectMetadataAsync(projectPath, options = {}) {
  ...
  const result = await runAsyncProbe(process.execPath, [path.join(codeRoot, "scripts/agents-mother/project-metadata-worker.mjs"), projectPath], {
    cwd: codeRoot, policy: "projectMetadata", timeout: options.timeoutMs,
  });

...
import { creationPendingKey, creationOperatorKey, readCreationOperator, creationRequestForAction, readCreationPending, creationPhaseLabel, creationStatusLabel, creationShouldPoll, creationResultPresentation, creationRecoveryExplanation } from "./creation-client-state";
...
export function AgentCreationProgress({ chatId, refreshKey }: { chatId: string; refreshKey?: string | number }) {
  ...
  return <section className="codex-operation-card" aria-label="Создание агента" style={{ marginBottom: 16, padding: 16 }}>
    ...
    {job.executionPolicy ? <details aria-label="Эффективные настройки задачи"><summary>Модель и пределы: {job.executionPolicy.modelId}</summary>
      ...
      <p>Контекст по документации провайдера: {job.executionPolicy.modelProfile?.declaredContextTokens?.toLocaleString('ru-RU') || 'не подтверждён'} токенов. Выходной предел приложения: {job.executionPolicy.modelProfile?.applicationOutputCap || 8192} токенов. Качество tools/schema проверяется отдельно.</p>
      <p>Настройка prompt budget {job.executionPolicy.configuredPromptTokenBudget.toLocaleString('ru-RU')} относится к обычному чату. Создание использует отдельные байтовые пределы и квоты подготовки.</p>
    </details> : <p>Историческая задача: сохраняет закреплённый executor и прежние ограничения.</p>}
    ...
    {job.observedUsage?.pendingRequests ? <p role="status">Ожидают завершения: {job.observedUsage.pendingRequests} запросов.</p> : null}
    {job.status==='running' && job.observedUsage?.providerProgress ? <p role="status" aria-label="Активность провайдера">Запрос провайдера: {job.observedUsage.providerProgress.state==='waiting'?'ожидается первый байт':job.observedUsage.providerProgress.state==='receiving'?'ответ поступает':job.observedUsage.providerProgress.state==='finished'?'ответ получен':'ответ прерван'}. Получено {((job.observedUsage.providerProgress.responseBytes||0)/1024).toFixed(1)} КиБ; первый байт {job.observedUsage.providerProgress.firstByteMs===null?'ещё не получен':`${Math.round(job.observedUsage.providerProgress.firstByteMs/1000)} с`}, последний {job.observedUsage.providerProgress.lastByteMs===null?'ещё не получен':`${Math.round(job.observedUsage.providerProgress.lastByteMs/1000)} с от начала запроса`}. Это признак активности, не подтверждение готовности результата.</p> : null}
    {job.observedUsage?.unknownRequests ? <p role="alert">Расход {job.observedUsage.unknownRequests} запросов не подтверждён после остановки. Новая отправка заблокирована; сохранённый результат доступен для проверки.</p> : null}
    {job.observedUsage?.unboundAttempts ? <p role="alert">Не удалось связать квитанции {job.observedUsage.unboundAttempts} исполнений. Итоговый расход остаётся неизвестным.</p> : null}
    {job.observedUsage?.reservedTokens ? <p>Удержанный резерв неизвестных запросов: {job.observedUsage.reservedTokens.toLocaleString("ru-RU")}. Это резерв, а не измеренный расход.</p> : null}
    ...
    {job.budget.unknownAttempts.length > 0 ? <p className="codex-inline-notice" role="alert">Есть исполнения с неподтверждённым расходом: {job.budget.unknownAttempts.length}. Новые запросы модели доступны только после подтверждения расхода. Сверка может оставить его неизвестным.</p> : null}
    ...
    {job.context ? <p aria-label="Размер контекста">{job.context.requestMode==='host-brief-v1'?'Brief без инструментов. ':job.context.requestMode==='host-research-v2'?'Research: выбор из сохранённых первоисточников без инструментов. ':job.context.requestMode==='host-research-v1'?'Research: инструкции только для проверки источников. ':''}Полный запрос: {job.context.bytes===null?'ещё не подготовлен':`${(job.context.bytes/1024).toFixed(1)} КиБ`}; резерв {job.context.reservation===null?'ещё не рассчитан':`${job.context.reservation.toLocaleString('ru-RU')} токенов`}. Границы: 64 КиБ для новой сессии, 96 КиБ для checkpoint, 128 КиБ — предел. КиБ измеряют байты; резерв не является фактическим расходом.</p> : null}
    {job.nextDispatch ? <p role="status">{job.nextDispatch.reason}</p> : null}
    {job.delivery?.recovery ? <p role="status">{creationRecoveryExplanation(job.delivery.recovery.reason)}</p> : null}

// Frozen pre-barrier writer from 44598b9cad637e3a7d81a373209610a476008176; only import paths are adapted.
// Used to verify safe refusal of the newer coordination schema.
...
export class NeuralDeepCoordinationStore {
  constructor({ databasePath = ":memory:", legacyPath = null, workerId = `worker_${randomUUID()}` } = {}) {
    ...
    if (![0, 1].includes(version)) throw new Error("admission_schema_unsupported");
    this.transaction(() => {
  ...
  claim(id, limit = 1) {
    ...
    return this.transaction(() => {
      ...
      if (row.session_scope) {
        ...
        const firstAlias = this.db.prepare(`SELECT id FROM attempts WHERE session_scope=? AND status='queued'
          AND (? IS NULL OR workload=?) ORDER BY priority,sequence LIMIT 1`).get(row.session_scope,owner?.owner || null,owner?.owner || null);
      ...
      const first = this.db.prepare("SELECT id FROM attempts WHERE scope=? AND status='queued' AND (? IS NULL OR workload=?) ORDER BY priority,sequence LIMIT 1")
  ...
  cancelQueued(id) {
    return this.db.prepare("UPDATE attempts SET status='cancelled',finished_at=? WHERE id=? AND status='queued'")

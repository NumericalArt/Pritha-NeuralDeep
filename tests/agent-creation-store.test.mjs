...
test('new research jobs pin topic policy 3, saved policy 2 survives replay, and updates cannot migrate either',()=>{
 const {coordination,store}=make();try {
  ...
  assert.equal(modern.researchSelectionVersion,3);
  ...
  const previous=store.create({...old,chatId:'passage_v2_chat',agentId:'passage-v2-agent',researchSelectionVersion:2});
  assert.equal(store.create({...previous,researchSelectionVersion:3}).researchSelectionVersion,2);
  for(const version of [0,4,'2',null])assert.throws(()=>store.create({...old,researchSelectionVersion:version}),{code:'creation_policy_invalid'});
